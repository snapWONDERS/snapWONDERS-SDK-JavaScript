...
import { uploadFile } from "./uploadRouter.js";
...
export class StegoSession {
   ...
   * cap can move server-side without an SDK release; undefined falls back to the built-in figure.
   */
  constructor(
    private readonly t: HttpTransport,
    readonly uploadUid: string,
  ...
  upload(filePath: string, step: number): Promise<string> {
    return uploadFile(this.t, filePath, this.uploadUid, step, this.maxUploadBytes);
  }
...
export class Stego {
  ...
  async createSession(sessionType: "hide" | "reveal"): Promise<StegoSession> {
    ...
    return new StegoSession(this.t, data.upload_uid, sessionType, data.max_upload_bytes);
  }
