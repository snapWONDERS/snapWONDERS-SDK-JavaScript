...
export class AnalyseItem {
  constructor(
     ...
     * `steganography` (hidden-content check), `forensic` (ELA / noise-inconsistency tile counts),
     * and — video only — `audio_splice` and `video_tamper` (inter-frame tamper, frame
     * duplication, P-frame FFT, GPS timezone triangle). A plain object — the exact keys grow over
     * time; read what you need.
     */
