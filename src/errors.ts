...
/**
 * An upload failed, by whichever method was used.
 *
 * There are two upload paths — a single-request direct POST and the resumable TUS protocol — and
 * which one runs is chosen for the caller by file size. Catching a method-specific error would
 * therefore mean catching one that only fires for some file sizes, so both throw this.
 */
export class UploadError extends SnapwondersError {}

/**
 * A TUS create/PATCH/resume step failed.
 *
 * Extends `UploadError` so `catch (e) { if (e instanceof UploadError) ... }` covers both paths.
 * Existing code catching `TusUploadError` keeps working.
 */
export class TusUploadError extends UploadError {}
